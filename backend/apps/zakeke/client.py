...
class ZakekeClient:
    ...
    def _get_access_token(self, access_type='C2S', visitorcode=None, customercode=None):
        ...
        # Note: We don't cache tokens with visitor/customer codes easily here, 
        # so for session-specific tokens we fetch fresh ones as per "Best Practice" in docs.
        # "Before you create another token, re-use the access token until it expires."
        # However, for C2S/S2S differentiation and user affinity, we usually call this per-request
        # if visitor/customer code is provided.
